function [cf0,dn]=trim_coef_3pix(cf0,sb,delta,T0,T)
...
while lg-ia>=3
   ...
   if (lenp1>lenp) & (lenp4>lenp)
      ...
      if ssb < th
         %[lenp1, lenp2,lenp3, lenp4] %%%%%%%%%%
         %[double(cf0(pos)) double(cf0(posA)) double(cf0(posB)) sb(pos) sb(posA) sb(posB)] %%%%%%%%%%%%%%%%%%%%%%
         cf0(pos)=0; cf0(posA)=0; cf0(posB)=0;
         gc(ia+3)=gc(ia+3)+lenp2+lenp3+lenp4;
