...
left_1s=sum(bin(ptr:lenbin));
while  n1s<=(n1s0-nrs)  &&  n1s<n1s0  &&  ptr+2+nrs<=lenbin && nrs<=left_1s %minimum of code is 3!
   ptr0=ptr; n1ss=n1s;
   [idx,b1s]=my_bin2dec(bin(ptr:ptr+2),3,power2s);  left_1s=left_1s-b1s;  ptr=ptr+3;
   if idx~=0  %When idx=0, do nothing (since codebook{1}=[0 0 0 0 0].)
      if bin(ptr)>0, idx=idx+idx+1; left_1s=left_1s-1;  else, idx = idx+idx; end;    ptr=ptr+1;
      if idx<=6
...
if flg==1 || ptr_inc==3 %i.e., an extra '00000' decoded <=== modified on 2017/11/15. ~~~~~~~~~~~~~~~~
