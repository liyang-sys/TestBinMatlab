function [qc,ptr]=de_coef2D_new_sub2(binary,H0,W0,ptr,delta)
...
hh=h; h=H0; ww=w; w=W0;
[r,c]=modify_Hilbert2D(h,w); rc=[r;c];
[qc(1:h,w+1:ww),ptr]=de_sub2d(binary,h,w,delta,rc,ptr);
[qc(h+1:hh,1:w),ptr]=de_sub2d(binary,h,w,delta,rc,ptr);
[qc(h+1:hh,w+1:ww),ptr]=de_sub2d(binary,h,w,delta,rc,ptr);
...
if binary(ptr)~=0
   ...
   offset=bin2dec(char( binary(ptr:ptr+6)+48 ))*D; ptr=ptr+7; %modified Oct 30, 2020
   [maxc,ptr] = deSFcode(binary,maxc,ptr);   %added Oct 30, 2020
   %====================== modified Oct 30, 2020 ====================================================================
else
   ptr=ptr+1;
end
