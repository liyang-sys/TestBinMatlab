function [m,stnew,L,Nsub,nsubtem,len]=modify_Hilbert3D_sub(dim,inc,m0,N0,Nsub,st0,nsubtem,L0)
...
if inc>0
   ...
   else %This part is different from 2D.
      len=N0+N0; r=int32(zeros(1,len)); c=r; d=r;   stnew=st+int32([0:(hN+qN):(hN+qN)*(nsub-1)]);
   end
else
   len=N0-nsub*hN; r=int32(zeros(1,len)); c=r; d=r;   stnew=st-int32([0:hN:hN*(nsub-1)]);
end
...
for ib=1:nsub
   ...
   if inc>0
      if dim-inc > 0
         ...
         ed0=ed; ednew0=ednew;
      else %This part is different from 2D. In this case, sr2A and sr4A are needed
         sr1=rsub0(1:qN);        sc1=csub0(1:qN);        sd1=dsub0(1:qN);
         sr2=sr1+inc/2;            %sc2=sc1, sd2=sd1, thus, use sc1 below directly.
         sr2=[sr2 sr1+inc];      sc2=[sc1 sc1];  sd2=[sd1 sd1]; 
         sr5=rsub0(hqN1:Nsub);   sc5=csub0(hqN1:Nsub);   sd5=dsub0(hqN1:Nsub);
         sr4=sr5+inc;            %sc4=sc5, sd4=sd5, thus, use sc5 below directly.
         sr4=[sr4 sr4-inc/2];      sc4=[sc5 sc5];  sd4=[sd5 sd5];
         
         rsub(1:qN)=sr1; rsub(qN1:qN+hN)=sr2; rsub(qN+hN1:qN+Nsub)=sr3; rsub(qN+Nsub1:Nq+hN)=sr4;  rsub(Nq1+hN:Nsub*2)=sr5;
         csub(1:qN)=sc1; csub(qN1:qN+hN)=sc2; csub(qN+hN1:qN+Nsub)=sc3; csub(qN+Nsub1:Nq+hN)=sc4;  csub(Nq1+hN:Nsub*2)=sc5;
         dsub(1:qN)=sd1; dsub(qN1:qN+hN)=sd2; dsub(qN+hN1:qN+Nsub)=sd3; dsub(qN+Nsub1:Nq+hN)=sd4;  dsub(Nq1+hN:Nsub*2)=sd5;
         snew=stnew(ib); ednew=stnew(ib)+Nsub+Nsub-1;
         r(ednew0:snew-1)=r0(ed0:s-1); r(snew:ednew)=rsub; 
         c(ednew0:snew-1)=c0(ed0:s-1); c(snew:ednew)=csub; 
         d(ednew0:snew-1)=d0(ed0:s-1); d(snew:ednew)=dsub; 
         ed0=ed; ednew0=ednew;
