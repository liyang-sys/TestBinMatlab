...
for n=1:len
   ...
   if subcf(row,col)~=0 
      rn=rn+1;
      if (map(row,col,1)==1  ||  rn0>0)  &&  map_new(row,col)==0
         rn0=rn0+1;
      end
   else
      if rn0>0
         if rn0>4
            ...
            for ia=1:rn
               ...
               if map_new(row,col)==0,   map_new(row,col)=1;   end
            end
            rnw=0;
         else
            flg=0;
            for ia=1:rn
               row=routxy(1,n-ia);  col=routxy(2,n-ia);
               if  map_new(row,col)==0  &&  abs(subcf(row,col))>1  &&  map(row,col,2)<=rn0
                  flg=1;
               end
            end
            if flg==1
