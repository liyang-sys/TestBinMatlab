...
%len=length(m); x=m(:,1:len-1)-m(:,2:len); x=sum(abs(x));  maxmin=[max(x) min(x)], len-Hn*Wn*2^k0
...
while finish==0 %change route direction:
   ...
   if Hn>=1
      ...
      if Wn>=2^k0
         ...
         Wn=sum(w(iw:length(w)));
         m=int32(zigzag3D(Hn,Wn,2^k0));
         finish=1;
      end
   else
      finish=1;
   ...
   if vertical>0
      ...
      c_offset=max(c1); %<=== c_offset needs to be modified
   end
   vertical=-vertical;
end
%----------------------------------------------------------------------------------------------------
