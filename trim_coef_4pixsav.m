function [cf0,dn]=trim_coef_4pix(cf0,sb,delta,T0,T)
...
while lg-ia>=4
   ...
   if (lenp1>lenp) & (lenp5>lenp)
      ...
      if ssb < th
         %[lenp1, lenp2,lenp3,lenp4, lenp5] %%%%%%%%%%
         %[double(cf0(pos)) double(cf0(posA)) double(cf0(posB)) sb(pos) sb(posA) sb(posB) sb(posC)] %%%%%%%%%%%%%%%%%%%%%%
         cf0(pos)=0; cf0(posA)=0; cf0(posB)=0; cf0(posC)=0;
         gc(ia+4)=gc(ia+4)+lenp2+lenp3+lenp4+lenp5;
