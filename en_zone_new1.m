...
if crs>60
   [bins2]=en_zone_sub_newsav(rs,crs,lBs);
   bin2=[1 bins2 bin_idx2 bink2];

   crw2=k_criterion(rw2);
   [binw]=en_zone_sub_newsav(rw2,crw2,[]);      
else
   %bins2=[lBs 0];
   ...
   bin2=[0];
   crw=k_criterion(rw) 
   [binw]=en_zone_sub_newsav(rw,crw,[]);      
end

lennew1=[length(bin1) length(bin2) length(binw)] %%%%%%%%%%%%%%%
%=======================================================================
