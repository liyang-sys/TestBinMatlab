...
for ia=(Nss+1): Ns
   ...
   cpropB=cpropA; pcB=1;   cpropC=cpropA; pcC=1;
   for ib=1:nn(1)
      for ic=1:nn(2)
         %if abs(subprop{1,ia-1}(ib,ic))>0
         if abs(subqcoef{1,ia-1}(ib,ic))>0
         ...
         if abs(subqcoef{ia-1,1}(ib,ic))>0
            tem=subqcoef{ia,1}(ib,ic);
            subprop{ia,1}(ib,ic)=tem;
            cpropB(pcB)=tem; pcB=pcB+1; 
         end
         
         if abs(subqcoef{ia-1,ia-1}(ib,ic))>0
            tem=subqcoef{ia,ia}(ib,ic);
            subprop{ia,ia}(ib,ic)=tem;
            cpropC(pcC)=tem; pcC=pcC+1; 
         end
      end
   end
   coefprop{1,ia}=cpropA(1:pcA-1);
   coefprop{ia,1}=cpropB(1:pcB-1);
   coefprop{ia,ia}=cpropC(1:pcC-1);
   ...
   fC=[sum(sum(abs(sign(subqcoef{ia,ia}))))/(nn2) sum(abs(sign(coefprop{ia,ia})))/length(coefprop{ia,ia})]
   maxC=[max(max(abs(subqcoef{ia,ia}))) max(abs(coefprop{ia,ia})) sum(sum(subqcoef{ia,ia}.^2)) sum(coefprop{ia,ia}.^2)] 
end
