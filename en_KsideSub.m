...
elseif wuns > len * (1 - 0.5^(1/5))
   [runs,lastBit,p]=Golomb0(c);
   bin=encode_stationary_source(runs,2); %codebook=1 is assumed here
   bin=[1 1 1 1 0 uint8(lastBit) bin inv];
elseif wuns == 0  %added Jan 5, 2021 <======================================= !!!!!!!!!!!!!!!!!!!
   bin=uint8([1 1 1 1 1 0 inv]);
   %========================================================================= !!!!!!!!!!!!!!!!!!!
else %wuns < len * (1 - 0.5^(1/5)),     but > len * (1 - 0.5^(1/7))
   [runs,lastBit,p]=Golomb0(c);
   bin=encode_stationary_source(runs,3); %codebook=2 is assumed here
   bin=[ 1 1 1 1 1 1 uint8(lastBit) bin inv];
   %fprintf('\n\n===> p may be outside the range in enKsideSub.m! %1.5f > %1.5f?  <======\n\n',wuns/len, 1 - 0.5^(1/7));
end
